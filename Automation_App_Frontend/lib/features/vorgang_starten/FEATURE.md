...
(`presentation/blocs/vorgang_starten_daten.dart`); verwendet werden `Vorgang`/`RechtsgebietWert`/`RegisterNummernStand`
...
`GET /api/Settings`, `POST /api/Mandanten`, `PUT /api/Mandanten/{id}`, `PUT /api/Vorgaenge`, `GET …/register/nummern`
