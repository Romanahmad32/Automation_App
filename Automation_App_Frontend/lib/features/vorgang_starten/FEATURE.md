...
**Anforderung:** `REQUIREMENTS.md` §4.1, §4.2, §6.3 (Vorgang als Klammer: §3)
...
(`presentation/blocs/vorgang_starten_daten.dart`); verwendet werden
`Vorgang`/`RechtsgebietWert`/`RegisterNummernStand` (vorgaenge), `Mandant`/`CreateMandantRequest`
(mandanten), `ZentralrufRequest` (zentralruf_request) und `KanzleiSettings` (settings) samt deren
UseCases.
...
- Der Auftragsnummer-Vorschlag kommt seit §6.3 aus dem Registerbestand, nicht mehr aus dem
  Zähler der Einstellungen (Rückfall bei Fehler); die Belegt-Warnung sperrt nicht — `FALLSTRICKE.md`.
...
- Der lange Rest steht in `FALLSTRICKE.md` daneben: Reihenfolge im Speicherpfad, die drei Zustände
  ...
  umbenennt (§5.1), das Kennzeichenfeld, die einfrierende Referenz-Vorschau, das Upsert hinter
  `registriereAnfrage` und warum Tests kein `pumpAndSettle` vertragen.
