...
`MailboxAccessView` in `settings` gepflegt. Auswerten und Zuordnen liegt in `zentralruf_reply`.
...
- `MailboxHub.ensureConnected()` schluckt einen fehlgeschlagenen Verbindungsaufbau still
  (`catch (_)`) — die Oberfläche meldet nichts, es bleibt nur „Aktualisieren". `withAutomaticReconnect()`
  lädt nach dem Wiederverbinden nichts nach: Treffer aus der Trennung erscheinen erst beim nächsten Signal.
...
- `ReceivedReply.zuordnungVermutet` kommt vom Backend, wird aber nirgends gelesen:
  `MailboxVorgangZuordnung` rechnet die Vermutung lokal über `findeWahrscheinlichenVorgang` neu aus.
...
- `MailboxVersandLeiste` gibt Vorgang **und** `reply.data` mit: Der Treffer ist hier noch nicht
  übernommen, die Versichereradresse steht also nur in der Antwort, nicht am Vorgang.
